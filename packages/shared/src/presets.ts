...
/**
 * Letters, numbers, spaces, dash, underscore, and dot only. Excludes Unicode
 * Punctuation broadly (the prior `\p{P}` admitted `<`, `>`, `&`, `"`, etc. —
 * safe today because Svelte auto-escapes, but a stored-XSS landmine if anyone
 * later renders a name via {@html} or copies it into an unescaped attribute).
 */
export const PresetNameSchema = z
  .string()
  .min(1)
  .max(64)
  .regex(/^[\p{L}\p{N}\p{Zs}\-_.]+$/u, "preset name contains invalid characters")
